      ...
      <Collapse isOpened={collapsed} className="block lg:hidden">
        <nav className="w-full h-auto mx-auto px-[5%] flex flex-col justify-center items-center lg:hidden">
          {navbarLinks.map((link) => (
            <Link key={link.name} to={link.path} className={`${location.pathname === link.path ? "underline underline-offset-2" : ""} m-2.5 text-gray-600 font-medium`} onClick={() => setCollapsed(false)}>
              {link.name}
            </Link>
          ))}
        </nav>
